...
export class SoundManager {
  ...
  constructor() {
    // Inicialização adiada para o primeiro clique do usuário
  }
  ...
  // Sintetizador de Caixa (Snare) para a Marcha Militar
  ...
  // Sintetizador de Morse para SFX
  public playMorse(type: 'move' | 'fusion' | 'start') {
    const ctx = this.init();
    const osc = ctx.createOscillator();
    const gain = ctx.createGain();

    osc.type = 'square'; // Som mais rústico de rádio antigo
  ...
  public startBGM() {
    ...
    let beat = 0;
    const tempo = 450; // BPM de marcha

    this.marchInterval = window.setInterval(() => {
      const now = ctx.currentTime;
      // Ritmo de marcha militar: 1 - 2 - 3&4
      if (beat === 0 || beat === 1) {
